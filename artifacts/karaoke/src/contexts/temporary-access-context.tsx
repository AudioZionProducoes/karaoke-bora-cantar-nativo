...
interface TemporaryAccessContextType {
  ...
  reactivateCode: (code: string, email?: string) => Promise<{ success: boolean; message?: string; error?: string }>;
  clearAccess: () => void;
}
...
export function TemporaryAccessProvider({ children }: { children: ReactNode }) {
  ...
  const reactivateCode = useCallback(async (code: string, email?: string): Promise<{ success: boolean; message?: string; error?: string }> => {
    try {
